...
const dlpPhases = [
  ...
  { num:"02", zone:"DISCOVER", zoneColor:"#7c3aed", color:"#7c3aed",
  ...
  { num:"03", zone:"DISCOVER", zoneColor:"#10b981", color:"#3b82f6", colorBg:(d)=>d?"rgba(59,130,246,0.07)":"rgba(59,130,246,0.06)", colorBorder:(d)=>d?"rgba(59,130,246,0.2)":"rgba(59,130,246,0.25)", title:"Security Team Alerts", subtitle:"SOC/DLP team notified — users remain unaware", icon:"🔔", impact:"low",
  ...
  { num:"04", zone:"EDUCATE", zoneColor:BUI_ORANGE, color:"#a78bfa", colorBg:(d)=>d?"rgba(167,139,250,0.07)":"rgba(167,139,250,0.06)", colorBorder:(d)=>d?"rgba(167,139,250,0.2)":"rgba(167,139,250,0.25)", title:"Policy Tips", subtitle:"Contextual guidance at the moment of risk", icon:"💡", impact:"moderate",
  ...
  { num:"07", zone:"GOVERN", zoneColor:BUI_GRAY, color:"#94a3b8", colorBg:(d)=>d?"rgba(148,163,184,0.07)":"rgba(148,163,184,0.06)", colorBorder:(d)=>d?"rgba(148,163,184,0.2)":"rgba(148,163,184,0.25)", title:"Continuous Improvement", subtitle:"Tuning, exception management & policy lifecycle", icon:"♻️", impact:"minimal",
...
const labelPhases = [
  ...
  { num:"04", zone:"EDUCATE", zoneColor:BUI_ORANGE, color:"#a78bfa", colorBg:(d)=>d?"rgba(167,139,250,0.07)":"rgba(167,139,250,0.06)", colorBorder:(d)=>d?"rgba(167,139,250,0.2)":"rgba(167,139,250,0.25)", title:"Recommended Labelling", subtitle:"Client-side suggestions based on content — dismissible", icon:"💡", impact:"moderate",
  ...
  { num:"07", zone:"GOVERN", zoneColor:BUI_GRAY, color:"#94a3b8", colorBg:(d)=>d?"rgba(148,163,184,0.07)":"rgba(148,163,184,0.06)", colorBorder:(d)=>d?"rgba(148,163,184,0.2)":"rgba(148,163,184,0.25)", title:"Auto-labelling & Governance", subtitle:"Service-side policies, label analytics, lifecycle management", icon:"♻️", impact:"minimal",
  ...
  function TrackRow({ phases, durations, cum, bound, trackLabel, trackColor, onPhaseClick }) {
    ...
    return (
      <div style={{ marginBottom:6 }}>
        {/* Track label */}
        <div style={{ fontSize:10, fontWeight:700, letterSpacing:"0.09em", color:trackColor, marginBottom:5, fontFamily:CAL }}>{trackLabel}</div>
            ...
            return (
              <div key={i}
                ...
                style={{
                  ...
                  opacity: covered ? 1 : 0.32,
                  border: isEdge ? `2px solid ${BUI_ORANGE}` : `1px solid ${covered ? p.color+"55" : t.border}`,
                  boxShadow: isEdge ? `0 0 0 3px ${BUI_ORANGE}22` : "none",
                  transition:"opacity 0.15s, border 0.15s",
                  display:"flex", flexDirection:"column",
                }}>
                {/* Zone colour band at top */}
                <div style={{ height:5, background: covered ? p.color : zColor, opacity: covered?1:0.4, flexShrink:0 }}/>
                {/* Content */}
                <div style={{
                  flex:1, background: covered ? `${p.color}18` : (dark?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.03)"),
                  padding:"5px 5px 3px", display:"flex", flexDirection:"column", justifyContent:"space-between",
                }}>
                  <div>
                    <div style={{ fontSize:9, fontWeight:700, color: covered?p.color:zColor, letterSpacing:"0.05em", lineHeight:1.2, fontFamily:CAL }}>{p.zone}</div>
                    <div style={{ fontSize:10, fontWeight:700, color: covered?t.text1:t.text4, lineHeight:1.25, marginTop:2, fontFamily:CAL }}>{p.title}</div>
                    {p.isPilot && <div style={{ fontSize:7.5, color:p.color, background:`${p.color}15`, padding:"1px 4px", borderRadius:99, display:"inline-block", marginTop:2, border:`1px solid ${p.color}40`, fontFamily:CAL }}>PILOT</div>}
                  </div>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                    <span style={{ fontSize:9, color: covered?t.text3:t.text5, fontFamily:CAL }}>{durations[i]}w</span>
                    <span style={{ fontSize:9, fontWeight:700, color: covered?p.color:t.text5, fontFamily:CAL }}>{p.num}</span>
                  </div>
                </div>
              </div>
            );
  ...
  return (
    <main style={{ flex:1, overflowY:"auto", padding:"24px 36px", background:t.bg, fontFamily:CAL }}>
      ...
      <div style={{ marginBottom:18 }}>
        ...
        <p style={{ fontSize:13, color:t.text4, margin:0, fontFamily:CAL }}>Phase widths are proportional to typical duration. Drag the orange line to set the BUI engagement boundary across both tracks simultaneously.</p>
      </div>
      ...
      <div style={{ background:t.bgCard, border:`1px solid ${t.border}`, borderRadius:14, padding:"20px 22px 16px", boxShadow:t.shadow, marginBottom:16 }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
          {[
            ...
            <div key={b.label} style={{ padding:"8px 14px", borderRadius:8, background:dark?`${b.color}0a`:`${b.color}08`, border:`1px solid ${b.color}25` }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:t.text1, fontFamily:CAL }}>{b.text}</span>
            </div>
          ))}
        </div>
        ...
        <div
          ...
          style={{ position:"relative", cursor: dragging?"grabbing":"grab", userSelect:"none", touchAction:"none" }}
        >
          {/* DLP track */}
          <TrackRow
          ...
          {/* ── VERTICAL DRAG LINE — rendered on top of both tracks ── */}
          <div style={{
          ...
          <div style={{ display:"flex", borderTop:`1px solid ${t.border}`, paddingTop:6 }}>
            {Array.from({length: Math.ceil(maxWeeks/4)+1}).map((_,i)=>(
              <div key={i} style={{ flex:1, maxWidth:`${4/maxWeeks*100}%` }}>
                <div style={{ fontSize:8.5, color:t.text5, fontFamily:CAL }}>w{i*4}</div>
              </div>
            ))}
          </div>
      ...
      <div style={{ display:"flex", gap:20, flexWrap:"wrap", marginBottom:16 }}>
        <div style={{ display:"flex", alignItems:"center", gap:6, fontSize:11, color:t.text3, fontFamily:CAL }}>
        ...
        {[{z:"DISCOVER",c:"#10b981"},{z:"EDUCATE",c:BUI_ORANGE},{z:"ENFORCE",c:"#ef4444"},{z:"GOVERN",c:BUI_GRAY}].map(z=>(
          <div key={z.z} style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:t.text3, fontFamily:CAL }}>
      ...
      {dlpBound >= 5 && labelBound >= 6 && (
        <div style={{ padding:"12px 16px", borderRadius:10, background:"rgba(16,185,129,0.08)", border:"1px solid rgba(16,185,129,0.25)", marginBottom:12 }}>
          <div style={{ fontSize:10, fontWeight:700, color:"#10b981", letterSpacing:"0.08em", marginBottom:4, fontFamily:CAL }}>CONVERGENCE SCOPE REACHED</div>
          <p style={{ fontSize:13, color:t.text3, margin:0, lineHeight:1.55, fontFamily:CAL }}>Both tracks are scoped to the convergence point. Label-conditioned DLP hard block policies are within scope of the engagement.</p>
        </div>
      )}
      {((dlpBound - labelBound) > 2) && (
        <div style={{ padding:"12px 16px", borderRadius:10, background:`${BUI_ORANGE}0a`, border:`1px solid ${BUI_ORANGE}30`, marginBottom:12 }}>
          <div style={{ fontSize:10, fontWeight:700, color:BUI_ORANGE, letterSpacing:"0.08em", marginBottom:4, fontFamily:CAL }}>TRACK ALIGNMENT NOTE</div>
          <p style={{ fontSize:13, color:t.text3, margin:0, lineHeight:1.55, fontFamily:CAL }}>DLP is significantly ahead of Labelling in scope. Hard DLP policies conditioned on sensitivity labels won't be effective until labelling reaches mandatory classification (phase 05).</p>
        </div>
      )}
      {((labelBound - dlpBound) > 2) && (
        <div style={{ padding:"12px 16px", borderRadius:10, background:`${BUI_ORANGE}0a`, border:`1px solid ${BUI_ORANGE}30`, marginBottom:12 }}>
          <div style={{ fontSize:10, fontWeight:700, color:BUI_ORANGE, letterSpacing:"0.08em", marginBottom:4, fontFamily:CAL }}>TRACK ALIGNMENT NOTE</div>
          <p style={{ fontSize:13, color:t.text3, margin:0, lineHeight:1.55, fontFamily:CAL }}>Labelling is significantly ahead of DLP in scope. Ensure DLP policies are progressed to use label conditions before broad encryption is applied.</p>
        </div>
      )}

      <div style={{ fontSize:11, color:t.text4, lineHeight:1.55, fontFamily:CAL }}>
        <span style={{ fontWeight:700, color:t.text3 }}>Tip: </span>
        Click any phase to jump to its detail. Drag the orange line left or right to adjust the engagement boundary across both tracks at once. Phase widths reflect typical duration — wider blocks take longer.
      </div>
              ...
              <div key={i}>
                ...
                <button onClick={()=>{setActive(i);setTab("overview");}} style={{ width:"100%", padding:"10px 14px", textAlign:"left", background:on?`${p.color}0e`:"transparent", border:"none", borderLeft:`3px solid ${on?p.color:"transparent"}`, cursor:"pointer", display:"flex", alignItems:"center", gap:10, transition:"all 0.14s" }}>
                  ...
                  <div style={{ minWidth:0, flex:1 }}>
                    ...
                    <div style={{ fontSize:10, color:p.color, opacity:0.75, marginTop:2 }}>{p.pilotLabel ? `→ ${p.pilotLabel.toLowerCase()}` : "→ continues to phase 03"}</div>
                  </div>
                </button>
                <div style={{ margin:"6px 14px", borderTop:`1px dashed ${p.color}50` }}/>
              </div>
            ...
            {phase.pilotWarning && (
              <div style={{ background:dark?"rgba(239,68,68,0.07)":"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.3)", borderLeft:"3px solid #ef4444", borderRadius:"0 10px 10px 0", padding:"14px 20px", marginBottom:18 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                  ...
                  <span style={{ fontSize:10, fontWeight:700, color:"#ef4444", letterSpacing:"0.1em" }}>IMPORTANT — PILOT COMPOSITION</span>
                </div>
                <p style={{ fontSize:13, color:t.text2, margin:0, lineHeight:1.65, fontFamily:CAL }}>{phase.pilotWarning}</p>
              </div>
            )}
            {phase.isPilot && (
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
                    {[
                      ...
                      <div key={i} style={{ padding:"10px 14px", borderRadius:8, background:dark?`${item.color}0a`:`${item.color}08`, border:`1px solid ${item.color}25` }}>
                        <div style={{ fontSize:10, fontWeight:700, color:item.color, letterSpacing:"0.08em", marginBottom:5 }}>{item.icon} {item.label.toUpperCase()}</div>
                        <p style={{ fontSize:12, color:t.text2, margin:0, lineHeight:1.55, fontFamily:CAL }}>{item.value}</p>
                      </div>
                    ))}
                  </div>
      ...
      <header style={{ height:62, padding:"0 24px", flexShrink:0, background:t.bgHeader, borderBottom:`1px solid ${t.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", boxShadow:t.shadow, transition:"background 0.25s" }}>
        <div style={{ display:"flex", alignItems:"center", gap:16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:10, color:BUI_GRAY, letterSpacing:"0.08em" }}>{view==="roadmap"?"ADOPTION ROADMAP":"MATURITY FRAMEWORK"}</div>
          </div>
          <div style={{ width:1, height:26, background:t.border }}/>
          {/* Track switcher */}
